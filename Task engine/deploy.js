...
const commands = `
cat << 'EOF' > /var/www/task-engine/check_tables.js
...
async function main() {
    ...
    try {
        const [rows1] = await connection.execute("DESCRIBE service_catalog");
        console.log("Service Catalog:", rows1);
        const [rows2] = await connection.execute("DESCRIBE task_generation_jobs");
        console.log("Task Generation Jobs:", rows2);
    } catch (e) {
        console.error(e);
    } finally {
...
main();
EOF
node /var/www/task-engine/check_tables.js
`;
