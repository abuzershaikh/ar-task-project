...
async function deployKycFix() {
  try {
    ...
    console.log('--- 1. Uploading updated KYC, profile & earning controllers to VPS ---');
    const localDir = path.resolve(__dirname, '..', 'Task engine');

    await ssh.putFile(
      path.join(localDir, 'apps', 'api', 'controllers', 'worker', 'kyc.controller.ts'),
      '/opt/task-engine/apps/api/controllers/worker/kyc.controller.ts'
    );
    console.log('✓ Uploaded kyc.controller.ts');
    ...
    await ssh.putFile(
      path.join(localDir, 'apps', 'api', 'controllers', 'worker', 'earning.controller.ts'),
      '/opt/task-engine/apps/api/controllers/worker/earning.controller.ts'
    );
    console.log('✓ Uploaded earning.controller.ts');
