...
async function run() {
  ...
  console.log('Firebase connection SUCCESS! Total users fetched:', users.size);
  users.forEach(u => console.log(' -> Document ID:', u.id, '| Role:', u.data().role, '| Email:', u.data().email || '(none)'));
}
run().then(() => process.exit(0)).catch(e => { console.error('Firebase error:', e); process.exit(1); });
`;
