...
async function deploy() {
  try {
    ...
    const testRevenue = await ssh.execCommand('curl -s -H "Authorization: Bearer test" http://127.0.0.1:3000/api/v1/admin/analytics/revenue');
    console.log('Revenue response:', testRevenue.stdout);

    const testLedger = await ssh.execCommand('curl -s -H "Authorization: Bearer test" http://127.0.0.1:3000/api/v1/admin/analytics/ledger');
    console.log('Ledger response:', testLedger.stdout);
