...
`;
  ...
  console.log('Writing /etc/nginx/conf.d/reviewsgateway.conf...');
  await ssh.execCommand(`cat << 'EOF' > /etc/nginx/conf.d/reviewsgateway.conf\n${confContent}\nEOF`);

  const t = await ssh.execCommand('nginx -t');
  console.log('nginx -t:', t.stdout || t.stderr);

  await ssh.execCommand('nginx -s reload');
  console.log('Nginx reloaded successfully with no-cache headers!');
