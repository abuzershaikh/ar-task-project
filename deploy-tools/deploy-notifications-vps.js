...
async function deployNotifications() {
  try {
...
admin.messaging().send(message)
  .then(resp => {
    console.log('✅ FCM Notification Test SUCCESS! Message ID:', resp);
    process.exit(0);
  })
  .catch(err => {
    console.error('❌ FCM Notification Test Error:', err);
    process.exit(1);
  });
`;

    const testRes = await ssh.execCommand(`node -e "${testScript.replace(/\n/g, ' ')}"`, { cwd: remoteBase });
    console.log(testRes.stdout);
    if (testRes.stderr) console.error(testRes.stderr);

    console.log('\n✅ Task Push Notification Engine deployed and verified on VPS!');
