...
class AppUpdateManagementScreen extends StatefulWidget {
  ...
  @override
  State<AppUpdateManagementScreen> createState() =>
...
class _AppUpdateManagementScreenState extends State<AppUpdateManagementScreen> {
  final TextEditingController _latestVersionController =
      TextEditingController(text: '1.0.5');
  final TextEditingController _latestVersionCodeController =
      TextEditingController(text: '5');
  final TextEditingController _apkUrlController =
    ...
    try {
      ...
      if (data != null && data['settings'] != null) {
        ...
        setState(() {
          ...
          _latestVersionController.text =
              (s['latestVersion'] ?? '1.0.5').toString();
          _latestVersionCodeController.text =
              (s['latestVersionCode'] ?? '5').toString();
          final rawUrl = (s['apkDownloadUrl'] ?? '').toString();
          _apkUrlController.text = (rawUrl.contains('github.com') ||
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Failed to load version settings: $e'),
  ...
  Future<void> _showAddVersionDialog() async {
    ...
    final defaultUrl = (_apkUrlController.text.contains('github.com') ||
            _apkUrlController.text.contains('raw.githubusercontent.com'))
        ? ''
        : _apkUrlController.text.trim();
    final linkCtrl = TextEditingController(text: defaultUrl);
    final msgCtrl =
        TextEditingController(text: 'Please update to continue using the app.');
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Version Code (Android Build Number) *',
                        style: TextStyle(
                            color: Color(0xFF334155),
                            fontSize: 11.5,
                            fontWeight: FontWeight.bold)),
  ...
  Future<void> _addVersion({
    ...
    try {
      final dio = getIt<DioClient>();
      final resp =
          await dio.post('/admin/settings/app-updates/add-version', data: {
      ...
      if (updatedList != null) {
        setState(() {
          _versionList =
              updatedList.map((e) => Map<String, dynamic>.from(e)).toList();
        });
      } else {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                '✅ Version "$versionName" (Code: $versionCode) added as ${status.toUpperCase()}!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Failed to add version: $e'),
              backgroundColor: Colors.redAccent),
        );
      }
    } finally {
  ...
  Future<void> _showDisableVersionDialog(Map<String, dynamic> item) async {
    ...
    final cleanItemUrl = (rawItemUrl.contains('github.com') ||
            rawItemUrl.contains('raw.githubusercontent.com'))
        ? ''
        : rawItemUrl;
    final defaultUrl = (_apkUrlController.text.contains('github.com') ||
            _apkUrlController.text.contains('raw.githubusercontent.com'))
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          ...
                          style:
                              TextStyle(color: Color(0xFF991B1B), fontSize: 11),
                        ),
                      ),
                    ],
                  ),
                ...
                const Text(
                    'New Latest Version Update Link (User Redirect URL) *',
                    style: TextStyle(
                        color: AppColors.primary,
                        fontSize: 11.5,
                        fontWeight: FontWeight.bold)),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'Enter APK download link or Play Store redirect URL...',
                    hintStyle:
          ...
          actions: [
            ...
            ElevatedButton.icon(
              ...
              label: const Text('Confirm & Disable',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              onPressed: () async {
                ...
                await _toggleStatus(item,
                    status: 'disabled', updateUrl: link, message: msg);
              },
            ),
          ],
        );
  ...
  Future<void> _toggleStatus(
    ...
    try {
      final dio = getIt<DioClient>();
      final resp =
          await dio.post('/admin/settings/app-updates/toggle-status', data: {
        'id': item['id'],
        'versionName': item['versionName'],
        'versionCode': item['versionCode'],
        'status': status,
        'updateUrl':
            updateUrl ?? item['updateUrl'] ?? _apkUrlController.text.trim(),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor:
                status == 'disabled' ? Colors.orangeAccent : Colors.green,
            behavior: SnackBarBehavior.floating,
          ),
        );
      }
  ...
  Future<void> _removeVersion(Map<String, dynamic> item) async {
    ...
    try {
      ...
      setState(() {
        _versionList.removeWhere(
            (e) => e['id'] == item['id'] || e['versionName'] == vName);
      });
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                '✅ App Version Registry & Configurations saved successfully!'),
            backgroundColor: Colors.green,
            behavior: SnackBarBehavior.floating,
          ),
        );
  ...
  Widget build(BuildContext context) {
    final disabledCount =
        _versionList.where((v) => v['status'] == 'disabled').length;
    final activeCount =
        _versionList.where((v) => v['status'] == 'active').length;
                ...
                children: [
                  ...
                  _buildSectionCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        const Text(
                            'Default APK Download URL (Opens in Chrome / In-App Downloader) *',
                            style: TextStyle(
                                color: Color(0xFF334155),
                                fontSize: 12,
                                fontWeight: FontWeight.bold)),
                        ...
                        TextFormField(
                          ...
                          style: const TextStyle(
                              color: Color(0xFF0F172A), fontSize: 12.5),
                          decoration: InputDecoration(
                            hintText:
                                'https://example.com/downloads/Worker_App.apk',
                            hintStyle: const TextStyle(
                                color: Color(0xFF94A3B8), fontSize: 12),
                            prefixIcon: const Icon(Icons.link_rounded,
                        ...
                        TextFormField(
                          ...
                          decoration: InputDecoration(
                            hintText:
                                'Message shown to worker on update screen...',
                            hintStyle: const TextStyle(
                                color: Color(0xFF94A3B8), fontSize: 11),
                            filled: true,
                            fillColor: const Color(0xFFF8FAFC),
                        ...
                        TextFormField(
                          ...
                          decoration: InputDecoration(
                            hintText:
                                'Bullet points explaining new features...',
                            hintStyle: const TextStyle(
                                color: Color(0xFF94A3B8), fontSize: 11),
                            filled: true,
                            fillColor: const Color(0xFFF8FAFC),
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            subtitle,
            style: const TextStyle(
                color: Color(0xFF64748B), fontSize: 11, height: 1.35),
          ),
